...
GET /v1/demographics/LK11                # 2024 census: age/sex/ethnicity/religion + 2012 deltas
GET /v1/population?lat=6.9271&lon=79.8612&radius=5
GET /v1/population/grid?res=0.02         # density buckets for the map heatmap
...
import { getPostalCode, nearestPostalCode } from "@lanka-data-layer/postal";

getUnit("LK11");                        // Colombo District, names in en/si/ta
nearestPostalCode(6.9271, 79.8612);     // → 00200 Slave Island, 1.21 km away
```

Each package exports `DATA_VERSION` and `SOURCES` so you always know which data build you're running and where it came from. The data files are generated by the same foundry pipeline that builds the API artifact, so packages and API never drift. GN-level reverse geocoding stays API-only (its 5.4M-cell lookup grid doesn't belong in a tarball).
